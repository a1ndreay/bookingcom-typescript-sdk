...
describe('instantiate client', () => {
  ...
  describe('logging', () => {
    ...
    beforeEach(() => {
      process.env = { ...env };
      process.env['BOOKING_COM_LOG'] = undefined;
    });
    ...
    const forceAPIResponseForClient = async (client: BookingCom) => {
    ...
    test('debug logs when log level is debug', async () => {
      ...
      const client = new BookingCom({ logger: logger, logLevel: 'debug', apiKey: 'My API Key' });
    ...
    test('debug logs are skipped when log level is info', async () => {
      ...
      const client = new BookingCom({ logger: logger, logLevel: 'info', apiKey: 'My API Key' });
    ...
    test('debug logs happen with debug env var', async () => {
      ...
      const client = new BookingCom({ logger: logger, apiKey: 'My API Key' });
    ...
    test('client log level overrides env var', async () => {
      ...
      process.env['BOOKING_COM_LOG'] = 'debug';
      const client = new BookingCom({ logger: logger, logLevel: 'off', apiKey: 'My API Key' });
  ...
  describe('baseUrl', () => {
    test('trailing slash', () => {
      const client = new BookingCom({ baseURL: 'http://localhost:5000/custom/path/', apiKey: 'My API Key' });
    ...
    afterEach(() => {
      process.env['BOOKING_COM_BASE_URL'] = undefined;
    });

    test('explicit option', () => {
      const client = new BookingCom({ baseURL: 'https://example.com', apiKey: 'My API Key' });
    ...
    test('empty env variable', () => {
      ...
      const client = new BookingCom({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://api.server.test/v1');
    });
  ...
  test('maxRetries option is correctly set', () => {
    const client = new BookingCom({ maxRetries: 4, apiKey: 'My API Key' });
    expect(client.maxRetries).toEqual(4);

    // default
    const client2 = new BookingCom({ apiKey: 'My API Key' });
    expect(client2.maxRetries).toEqual(2);
  });

  test('with environment variable arguments', () => {
    // set options via env var
    process.env['BOOKING_COM_API_KEY'] = 'My API Key';
    const client = new BookingCom();
  ...
  test('with overridden environment variable arguments', () => {
    // set options via env var
    process.env['BOOKING_COM_API_KEY'] = 'another My API Key';
    const client = new BookingCom({ apiKey: 'My API Key' });
    expect(client.apiKey).toBe('My API Key');
  });
});
...
describe('retries', () => {
  test('retry on timeout', async () => {
    ...
    const client = new BookingCom({ apiKey: 'My API Key', timeout: 10, fetch: testFetch });
  ...
  test('retry count header', async () => {
    ...
    const client = new BookingCom({ apiKey: 'My API Key', fetch: testFetch, maxRetries: 4 });
  ...
  test('retry on 429 with retry-after', async () => {
    ...
    const client = new BookingCom({ apiKey: 'My API Key', fetch: testFetch });
